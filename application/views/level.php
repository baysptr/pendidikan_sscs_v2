    ...
    <body class="hold-transition skin-blue sidebar-mini">
                                ...
                                <tbody>
                                    <?php foreach ($data_level as $level) { ?>
                                        <tr>
                                            <td><?= $level['level'] ?></td>
                                            <td width="10%">
                                                <div class="btn btn-xs btn-block btn-success" onclick="show_menu('<?= $level['id'] ?>')"><span class="glyphicon glyphicon-link"></span> Link Menu</div>
                                            </td>
                                            <td><div class="btn btn-xs btn-danger" onclick="javascript:
                                                            if (confirm('Apakah Anda yakin akan menghapus ini ???') === true) {
                                                        window.location = '<?= site_url() ?>/admin/delete_level/<?= $level['id'] ?>';
                                                                }
                                                     "><span class="glyphicon glyphicon-trash"></span></div></td>
                                            <td><div class="btn btn-xs btn-primary" onclick="show('<?= $level['id'] ?>')"><span class="glyphicon glyphicon-edit"></span></div></td>
                                        </tr>
                                    <?php } ?>
                                </tbody>
                            </table>
        ...
        <script type="text/javascript">
                                            ...
                                            function show_menu(id){
                                                window.location = "<?= site_url() ?>/admin/detail_level/" + id;
                                            }
                                            
                                            function add() {
                                                method = "add";
                                                $(".modal-title").text("Form Tambah Level");
                                                $("#myModal").modal("show");
                                            }
                                            ...
                                            function show(id) {
                                                ...
                                                $.ajax({
                                                    url: "<?= site_url('admin/show_level') ?>/" + id,
                                                    type: 'GET',
                                                    dataType: 'JSON',
                                                    success: function (data) {
                                                        $(".modal-title").text("Form Ubah Level");
                                                        $('#id').val(data.id);
                                            ...
                                            function save() {
                                                var url;
                                                if (method === "add") {
                                                    url = "<?= site_url() ?>/admin/add_level";
                                                } else {
                                                    url = "<?= site_url() ?>/admin/update_level";
                                                }
