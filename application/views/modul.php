...
<html>
    ...
    <body class="hold-transition skin-blue sidebar-mini">
                            ...
                            <table class="table table-condensed table-hover">
                                ...
                                <tbody>
                                    ...
                                    <tr>
                                        ...
                                        <td><div class="btn btn-xs btn-danger" onclick="javascript: 
                                                 if(confirm('Apakah Anda yakin akan menghapus ini ???') === true){
                                                     window.location = '<?= site_url() ?>/admin/delete_modul/<?= $modul['id'] ?>';
                                                 }
                                                 "><span class="glyphicon glyphicon-trash"></span></div></td>
                                        <td><div class="btn btn-xs btn-primary" onclick="show('<?= $modul['id'] ?>')"><span class="glyphicon glyphicon-edit"></span></div></td>
                                        <td>
                                            <div class="btn btn-xs btn-warning" onclick="window.location = '<?= site_url() ?>/admin/parameter/<?= $modul['id'] ?>';">
                                                Add parameter
                                            </div>
                                        </td>
                                    </tr>
                        ...
                        <form id="form" method="post" enctype="multipart/form-data">
                            ...
                            <table class="table table-responsive table-hover table-condensed">
                                <tr>
                                    <td><b>Nama Modul</b></td>
                                    <td>:</td>
                                    <td><input type="text" name="nama_modul" id="nama_modul" class="form-control"></td>
                                </tr>
                                <tr>
                                    <td><b>Deskripsi Modul</b></td>
                                    <td>:</td>
                                    <td>
                                        <textarea name="deskripsi_modul" id="deskripsi_modul" class="form-control"></textarea>
                                    </td>
                                </tr>
                                <tr>
                                    <td><b>File Modul</b></td>
                                    <td>:</td>
                                    <td>
                                        <input type="file" name="file_modul" id="file_modul">
                                        <br/>
                                        <input type="checkbox" id="check" name="check" value="1"> <span id="title_check">Centang untuk ganti file</span>
                                    </td>
                                </tr>
                                <tr>
                                    <td colspan="2" align="right">
                                        <button class="btn btn-sm btn-danger" onclick="close()"><span class="glyphicon glyphicon-backward"></span> Back</button>
                                        <button class="btn btn-sm btn-primary" onclick="save()"><span class="glyphicon glyphicon-save"></span> Save</button>
                                    </td>
                                    <td>&nbsp;</td>
                                </tr>
                            </table>
        ...
        <script type="text/javascript">
            ...
            function add(){
                method = "add";
                $(".modal-title").text("Form Tambah Modul");
                $("#check").hide();
                $("#title_check").hide();
                $("#myModal").modal("show");
            }
            ...
            function show(id){
                ...
                $.ajax({
                    url: "<?= site_url('admin/show_modul') ?>/" + id,
                    type: 'GET',
                    dataType: 'JSON',
                    success: function (data) {
                        $(".modal-title").text("Form Ubah Modul");
                        $('#id').val(data.id);
                        $('#nama_modul').val(data.nama_modul);
                        $('#deskripsi_modul').val(data.deskripsi_modul);
                        $("#check").show();
                        $("#title_check").show();
                        $('#myModal').modal('show');
                    }
            ...
            var url;
                if(method === "add"){
                    url = "<?= site_url() ?>/admin/add_modul";
                }else{
                    url = "<?= site_url() ?>/admin/update_modul";
                }
